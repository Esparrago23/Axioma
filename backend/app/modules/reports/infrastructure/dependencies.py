...
from app.modules.notifications.infrastructure.dependencies import get_send_push_uc, get_send_evolution_confirmed_uc
...
def get_evolutions_uc(repo=Depends(get_reports_repo)): return GetEvolutionsUseCase(repo)
def get_create_evolution_uc(repo=Depends(get_reports_repo)): return CreateEvolutionUseCase(repo)
def get_vote_evolution_uc(
    repo=Depends(get_reports_repo),
    notify_uc=Depends(get_send_evolution_confirmed_uc),
): return VoteEvolutionUseCase(repo, notify_uc)
